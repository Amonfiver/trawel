...
**Ruta Comunidad:** existe `/comunidad` en `src/pages/CommunityPage/` y el header global enlaza a ella como `Comunidad`. La pagina inicial es un estado publico controlado: presenta el espacio y envia propuestas a `/compartir`, pero todavia no lee una tabla publica definitiva ni muestra aportes pendientes. Desde el bloque 90 usa lenguaje de produccion online: seleccion editorial inicial y ejemplos editoriales mientras llegan aportaciones reales aprobadas.

**Cards Comunidad:** `/comunidad` muestra cards visuales locales de seleccion editorial inicial con foto, titulo, pais, zona cuando aplica, tipo de aporte, resumen y credito. Son ejemplos editoriales temporales; no representan contenido de usuario publicado ni deben confundirse con una cola pendiente. Si una card tiene `country_slug` y `zone_slug`, enlaza a `/pais/{countrySlug}/zona/{zoneSlug}`; si solo tiene `country_slug`, enlaza a `/pais/{countrySlug}`. Nada enviado por usuarios aparece sin revision, clasificacion y aprobacion de Investighost.
...
**Railes de coste:** `docs/TRAWEL_COST_GUARDRAILS.md` documenta el Bloque 85. La migracion `011_create_cost_guardrails.sql` crea `system_flags`, `storage_usage_events` y `moderation_cleanup_queue`. Flags clave: contacto/compartir/reportes pueden apagarse, `photo_uploads_enabled=false`, maximo 3 fotos por envio futuro, limites por email hash y caducidad de pendientes/rechazados. No abrir Storage ni activar fotos reales sin Edge Function segura y limites cerrados.

**Rate limit publico:** el Bloque 86 crea `public_submission_events` con la migracion `012_create_public_submission_events.sql` y actualiza `protected-public-submit`. La Edge Function hashea email normalizado, hashea IP solo si hay cabecera disponible, lee `max_hourly_submissions_per_email` y `max_daily_submissions_per_email` desde `system_flags`, registra eventos aceptados/rechazados y devuelve `429` amable si se supera limite. No guardar emails en claro en eventos.

**Retencion y limpieza:** `docs/TRAWEL_RETENTION_AND_CLEANUP_POLICY.md` documenta el Bloque 87. La migracion `013_create_cleanup_helpers.sql` crea vistas internas de pendientes/rechazados antiguos y la funcion manual `enqueue_expired_pending_items()`, que solo encola en `moderation_cleanup_queue`. No borra datos automaticamente, no toca Storage y no activa cron externo.

**Flujo futuro fotos:** `docs/TRAWEL_PHOTO_UPLOAD_GUARDED_FLOW.md` documenta el Bloque 88. `/compartir` puede estandarizar fotos localmente, pero no sube archivos. `photo_upload_pending=true` solo marca que hay fotos pendientes de flujo futuro. `photo_uploads_enabled=false` sigue siendo el cierre global. No crear ni desplegar `guarded-photo-upload`, no abrir Storage y no activar subida real sin bloque explicito.

**Revision seguridad/costes pre-hosting:** `docs/TRAWEL_SECURITY_COST_PRE_HOSTING_REVIEW.md` cierra el Bloque 89. Antes de hosting real aplicar migraciones 011-013 con `npx supabase db push --linked` y desplegar `protected-public-submit` porque fue tocada en el Bloque 86. Confirmado localmente: build OK, rutas criticas OK, formularios bloquean sin token y envian con token simulado, fotos reales desactivadas, `dist/.htaccess` debe recrearse tras cada build.
...
**Contrato catalogo pais/ciudad:** `docs/TRAWEL_COUNTRY_CITY_CATALOG_CONTRACT.md` define campos minimos/recomendados para `countries` y `cities`: `id`, `slug`, `name_es`, estado, visibilidad publica, featured, tipo de zona, coordenadas si procede y descripciones breves. Slugs estables, sin duplicados dentro del mismo pais; Trawel solo lee y no inventa catalogo.
