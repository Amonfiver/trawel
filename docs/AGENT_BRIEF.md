...
**Home data-driven inicial:** `HomePage` consume `getResolvedHomeScreenData(mode)` para hero, destinos destacados, planes destacados y CTA comunidad. Puede intentar paises destacados desde `countries` legacy (`status` publico y `featured=true`) y mantiene `getHomeScreenFallbackData(mode)` como fallback local inmediato. No cambia `WorldMap`.
